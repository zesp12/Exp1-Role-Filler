...
const inPairIndex = Math.floor(Math.random() * objectPairIDs.length);
let onPairIndex = Math.floor(Math.random() * (objectPairIDs.length - 1));
if (onPairIndex >= inPairIndex) onPairIndex += 1; 

const inTargetImage = containmentImage(objectPairIDs[inPairIndex]);
const onTargetImage = supportImage(objectPairIDs[onPairIndex]);

const inEpoch = { type: "IN", targetImage: inTargetImage, pairID: objectPairIDs[inPairIndex] };
const onEpoch = { type: "ON", targetImage: onTargetImage, pairID: objectPairIDs[onPairIndex] };

const epochOrder = shuffle([inEpoch, onEpoch]);
const epoch1 = epochOrder[0];
const epoch2 = epochOrder[1];
...
function buildEpochTrialList(epochTargetImage, otherEpochTargetImage) {
    ...
    for (let b = 0; b < subBlocksPerEpoch; b++) {
        ...
        const taggedSubBlock = shuffledSubBlock.map(img => ({ image: img, subBlock: subBlockNumber }));
        epochList = epochList.concat(taggedSubBlock);
    }
    return epochList;
}

const epoch1Trials = buildEpochTrialList(epoch1.targetImage, epoch2.targetImage);
const epoch2Trials = buildEpochTrialList(epoch2.targetImage, epoch1.targetImage);
